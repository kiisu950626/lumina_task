...
class DataTrainingArguments:
    ...
    streaming: Optional[bool] = field(
        default=True,
        metadata={"help": ("Dataset is stream mode or not " "value if set.")},
    )
    ...
    corpus_data_dir: str = field(
        default=None,
        metadata={"help": ("local common voice path ")},
    )
...
def main():
    ...
    def load_maybe_streaming_dataset(
        ...
        **kwargs,
        ...
        print("split: ", split)
        datas = {
            split_name: f"{data_dir}/{dataset_config_name}/{split_name}*.tsv"
        ...
        if "+" in split:
            ...
            dataset_splits = load_dataset(
                dataset_name, data_files=datas, sep=sep, streaming=streaming, **kwargs
            )
    ...
    print("audio column name: ", raw_datasets.values())
    if (
        data_args.audio_column_name
